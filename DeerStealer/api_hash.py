...
RULE_SOURCE_CONSTANT = """rule HijackLoaderConstant
{
    meta:
        author = "Yung Binary"
...
RULE_SOURCE_API_HASH = """rule HijackLoaderApiHash
{
    meta:
        author = "Yung Binary"
...
def build_map(constant):
    export_map = {}
    for api_name in KERNEL32_EXPORT_NAMES:
        export_map[api_name] = custom_hash(constant, api_name)
    return export_map


def custom_hash(constant: int, export_name: str) -> int:
...
# Find the constant for API hashing
constant = 0
for match in yara_scan(data, RULE_SOURCE_CONSTANT):
    rule_str_name, match_offset = match
    displacement_value = data[match_offset + 2 : match_offset + 6]
    match_mem_offset = file_offset_to_memory_offset(pe, match_offset)
    dword_offset = memory_address_to_file_offset(pe, match_mem_offset + struct.unpack("i", displacement_value)[0] + 6)
    dw_constant = struct.unpack("<I", data[dword_offset: dword_offset + 4])[0]
    constant = dw_constant
    break

# Build a map of API names to hashes using the found constant
api_map = build_map(constant)

# Find API hashes (referenced as DWORDs) and
# convert them to their associated API name and
# set a comment
for match in yara_scan(data, RULE_SOURCE_API_HASH):
    try:
        rule_str_name, match_offset = match
        displacement_value = data[match_offset + 3 : match_offset + 7]
        match_mem_offset = file_offset_to_memory_offset(pe, match_offset)
        dword_offset = memory_address_to_file_offset(pe, match_mem_offset + struct.unpack("i", displacement_value)[0] + 7)
        api_hash = struct.unpack("<I", data[dword_offset: dword_offset + 4])[0]
        api_name = next((k for k, v in api_map.items() if v == api_hash))
        print(f"Found hash {hex(api_hash)} at offset {hex(match_mem_offset + image_base)}, resolved API name: {api_name}")
        ea = idaapi.get_fileregion_ea(match_offset)
        idc.set_cmt(ea, api_name, 0)
    except:
        continue
