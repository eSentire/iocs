...
// Handle stager obfuscation
if (isStager(ast)) {
  console.log('Running deobfuscation for stager payload...');
  ast = handleStager(ast);
}
// Handle primary backdoor/RAT obfuscation
else {
  console.log('Running deobfuscation for primary payload...');
  ast = handleStage3(ast);
}


function handleStager(ast) {
  const firstNode = ast.program.body[0];
  // The variable that stores the unshuffled strings array
  const variableName = firstNode.declarations[0].id.name;
  // The call expression that leads to the unshuffled strings array
  const callExpression = firstNode.declarations[0].init;
  // The first argument of the call expression (the shuffled strings)
  const shuffledString = callExpression.arguments[0].value;
  // The second argument of the call expression (the key)
  const key = callExpression.arguments[1].value;
  
  function unshuffleStringToArray(o, c) {
    var d = o.length;
    var u = [];
    for (var m = 0; m < d; m++) {
      u[m] = o.charAt(m);
    ...
    ;
    for (var m = 0; m < d; m++) {
      var t = c * (m + 226) + c % 47063;
      var a = c * (m + 576) + c % 47959;
      var f = t % d;
      var n = a % d;
      var h = u[f];
      u[f] = u[n];
    ...
    var i = '#0';
    var x = '#';
    return u.join(w).split(y).join(s).split(p).join(j).split(i).join(x).split(s);
  }

  let unshuffledArray = unshuffleStringToArray(shuffledString, key);
  
  // Using the resolved string array, replace array index lookups with string literals
  traverse(ast, {
    MemberExpression(path) {
      // Ensure variable name is what we previously identified
      if (!t.isIdentifier(path.node.object, { name: variableName })) {
        return;
      }
        
      // Check for bracket notation with numeric literal, e.g. _$_5c04[0]
      if (!path.node.computed || !t.isNumericLiteral(path.node.property)) {
        return;
      }

      // Lookup the string by index and replace it with the resolved string literal
      const index = path.node.property.value;
      const stringLiteral = unshuffledArray[index];
      path.replaceWith(t.stringLiteral(stringLiteral));
      console.log(`Replaced ${variableName}[${index}] with '${stringLiteral}'.`);
    }
  });
...
function isStager(ast) {
  const firstNode = ast.program.body[0];
  // First node should be a variable declaration of kind 'var'
  if (!t.isVariableDeclaration(firstNode, { kind: 'var' })) return false;

  // Value of the variable should be a call expression with 2 parameters
  // first is a string literal, second is numeric literal
  const callExpression = firstNode.declarations[0].init;
  if (!t.isCallExpression(callExpression) || callExpression.arguments.length !== 2) return false;

  const [arg1, arg2] = callExpression.arguments;

  // Ensure the first argument is a string > 50 characters
  const isLargeString = t.isStringLiteral(arg1) && arg1.value.length > 50;

  // Ensure the second arg is a numeric literal
  const isNumeric = t.isNumericLiteral(arg2);

  return isLargeString && isNumeric;
}
